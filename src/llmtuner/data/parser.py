...
def get_dataset_list(data_args: "DataArguments", split='train') -> List["DatasetAttr"]:
    if split == 'train':
        dataset_names = [ds.strip() for ds in data_args.dataset.split(",")] if data_args.dataset is not None else []
    else:
        dataset_names = [ds.strip() for ds in data_args.eval_dataset.split(",")] if data_args.eval_dataset is not None else []
    try:
